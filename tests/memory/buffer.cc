...
void WriteTestPattern(const Buffer& buf, uint8_t pattern) {
    if (buf.device() == DeviceType::CPU) {
        ...
        if (contents) {
            std::memset(contents, pattern, buf.sizeBytes());
        }
    }
#endif
}
...
TEST_CASE("Buffer Creation and Destruction", "[buffer][creation]") {
    SECTION("Default Constructor State") {
        ...
        REQUIRE(buf.device() == DeviceType::None);
        REQUIRE(buf.nativeDevice() == DeviceType::None);
        REQUIRE(buf.location() == Location::None);
        REQUIRE_FALSE(buf.isBorrowed());
        REQUIRE(buf.data() == nullptr);
    }

    SECTION("CPU Creation - Empty") {
        Buffer buf;
        REQUIRE(buf.create(DeviceType::CPU, 0) == Result::SUCCESS);
        REQUIRE(buf.valid());
        REQUIRE(buf.sizeBytes() == 0);
        REQUIRE(buf.device() == DeviceType::CPU);
        REQUIRE(buf.nativeDevice() == DeviceType::CPU);
    ...
    SECTION("CPU Creation - Non-Empty") {
        Buffer buf;
        REQUIRE(buf.create(DeviceType::CPU, TEST_SIZE) == Result::SUCCESS);
        REQUIRE(buf.valid());
        REQUIRE(buf.sizeBytes() == TEST_SIZE);
        REQUIRE(buf.device() == DeviceType::CPU);
        REQUIRE(buf.nativeDevice() == DeviceType::CPU);
        REQUIRE(buf.location() == Location::Host);
        REQUIRE_FALSE(buf.isBorrowed());
    ...
    SECTION("Metal Creation - Empty") {
        Buffer buf;
        REQUIRE(buf.create(DeviceType::Metal, 0) == Result::SUCCESS);
        REQUIRE(buf.valid());
        REQUIRE(buf.sizeBytes() == 0);
        REQUIRE(buf.device() == DeviceType::Metal);
        REQUIRE(buf.nativeDevice() == DeviceType::Metal);
    ...
    SECTION("Metal Creation - Non-Empty") {
        Buffer buf;
        REQUIRE(buf.create(DeviceType::Metal, TEST_SIZE) == Result::SUCCESS);
        REQUIRE(buf.valid());
        REQUIRE(buf.sizeBytes() == TEST_SIZE);
        REQUIRE(buf.device() == DeviceType::Metal);
        REQUIRE(buf.nativeDevice() == DeviceType::Metal);
    ...
    SECTION("Double Creation") {
        Buffer buf;
        REQUIRE(buf.create(DeviceType::CPU, TEST_SIZE) == Result::SUCCESS);
        REQUIRE(buf.create(DeviceType::CPU, TEST_SIZE) == Result::ERROR);  // Already initialized
...
TEST_CASE("Buffer Copy Operations", "[buffer][copy]") {
    SECTION("CPU to CPU - Fresh Copy") {
        Buffer src;
        REQUIRE(src.create(DeviceType::CPU, TEST_SIZE) == Result::SUCCESS);
        WriteTestPattern(src, 0xCC);

        Buffer dst;
        REQUIRE(dst.copyFrom(src) == Result::SUCCESS);
        REQUIRE(dst.valid());
        REQUIRE(dst.sizeBytes() == TEST_SIZE);
        REQUIRE(dst.device() == DeviceType::CPU);
        REQUIRE(dst.nativeDevice() == DeviceType::CPU);
    ...
    SECTION("CPU to CPU - To Existing Buffer") {
        ...
        Buffer dst;
        REQUIRE(dst.create(DeviceType::CPU, TEST_SIZE) == Result::SUCCESS);
    ...
    SECTION("Mismatched Sizes") {
        ...
        Buffer dst;
        REQUIRE(dst.create(DeviceType::CPU, TEST_SIZE * 2) == Result::SUCCESS);
        REQUIRE(dst.copyFrom(src) == Result::ERROR);  // Size mismatch
    }
...
#ifdef JETSTREAM_BACKEND_METAL_AVAILABLE
        Buffer dst;
        REQUIRE(dst.create(DeviceType::Metal, TEST_SIZE) == Result::SUCCESS);
        REQUIRE(dst.copyFrom(src) == Result::ERROR);  // Device mismatch
#endif
    }

    SECTION("Empty Buffer Copy") {
        Buffer src;
        REQUIRE(src.create(DeviceType::CPU, 0) == Result::SUCCESS);

        Buffer dst;
        REQUIRE(dst.copyFrom(src) == Result::SUCCESS);
        REQUIRE(dst.sizeBytes() == 0);
        REQUIRE(dst.device() == DeviceType::CPU);
        REQUIRE(dst.valid());
    }
    ...
    SECTION("Metal to CPU Copy") {
        Buffer src;
        REQUIRE(src.create(DeviceType::Metal, TEST_SIZE) == Result::SUCCESS);
        WriteTestPattern(src, 0xEE);

        Buffer tmp;
        REQUIRE(tmp.create(DeviceType::CPU, src) == Result::SUCCESS);

        Buffer dst;
        REQUIRE(dst.create(DeviceType::CPU, TEST_SIZE) == Result::SUCCESS);
        REQUIRE(dst.copyFrom(tmp) == Result::SUCCESS);
        REQUIRE(dst.device() == DeviceType::CPU);
        REQUIRE(dst.sizeBytes() == TEST_SIZE);
        REQUIRE_FALSE(dst.isBorrowed());
    ...
    SECTION("CPU to Metal Copy") {
        ...
        Buffer tmp;
        REQUIRE(tmp.create(DeviceType::Metal, src) == Result::SUCCESS);
...
TEST_CASE("Buffer Mirroring", "[buffer][mirror]") {
    ...
    SECTION("Mirror from Invalid Buffer") {
        Buffer src;  // Not initialized
        Buffer dst;
        REQUIRE(dst.create(DeviceType::CPU, src) == Result::ERROR);
        REQUIRE_FALSE(dst.valid());
    }
    ...
    SECTION("Metal to CPU Mirror - Unified") {
        ...
        REQUIRE(dst.valid());
        REQUIRE(dst.device() == DeviceType::CPU);
        REQUIRE(dst.nativeDevice() == DeviceType::Metal);  // Should inherit native device
        REQUIRE(dst.sizeBytes() == TEST_SIZE);
        REQUIRE(dst.isBorrowed());  // Should be borrowed from unified
    ...
    SECTION("Double Mirror Creation") {
        ...
        Buffer dst;
        REQUIRE(dst.create(DeviceType::Metal, src) == Result::SUCCESS);
        REQUIRE(dst.create(DeviceType::Metal, src) == Result::ERROR);  // Already initialized
...
TEST_CASE("Buffer Edge Cases", "[buffer][edge]") {
    SECTION("Large Allocation") {
        ...
        Buffer buf;
        REQUIRE(buf.create(DeviceType::CPU, large_size) == Result::SUCCESS);
        REQUIRE(buf.sizeBytes() == large_size);
        void* ptr = buf.data();
        REQUIRE(ptr != nullptr);
    ...
    SECTION("Copy Constructor and Assignment") {
        ...
        Buffer copy1(src);
        REQUIRE(copy1.valid());
        REQUIRE(copy1.sizeBytes() == TEST_SIZE);
        ...
        copy2 = src;
        REQUIRE(copy2.valid());
        REQUIRE(copy2.sizeBytes() == TEST_SIZE);
        REQUIRE(VerifyTestPattern(copy2, 0x77));

        // Move constructor
        Buffer moved(std::move(copy1));
        REQUIRE(moved.valid());
        REQUIRE(moved.sizeBytes() == TEST_SIZE);
        ...
        REQUIRE(moved2.sizeBytes() == TEST_SIZE);
