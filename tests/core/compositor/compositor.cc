...
TEST_CASE("Compositor command diagnostics belong to one completion",
           "[core][compositor][worker][result][error]") {
    JST_LOG_LAST_ERROR().clear();

    CompositorWorkerDouble worker;
    worker.start();
    worker.submit([] {
        JST_LOG_LAST_ERROR() = "first command error";
        return Result::ERROR;
    });
    worker.submit([] {
        return Result::ERROR;
    ...
    CompositorWorkerDouble::CompletedCommand command;
    REQUIRE(worker.take(command));
    REQUIRE(command.result == Result::ERROR);
    REQUIRE(command.message == "first command error");

    REQUIRE(worker.take(command));
    REQUIRE(command.result == Result::ERROR);
    // Current defect: the process-global last error leaks into a later command.
    REQUIRE(command.message.empty());
    REQUIRE_FALSE(worker.take(command));
}

TEST_CASE("Compositor warning and fatal diagnostics belong to one completion",
           "[core][compositor][worker][result]") {
    Result result = Result::WARNING;
    std::string* diagnostic = &JST_LOG_LAST_WARNING();

    SECTION("warning") {
        result = Result::WARNING;
        diagnostic = &JST_LOG_LAST_WARNING();
    }

    SECTION("fatal") {
        result = Result::FATAL;
        diagnostic = &JST_LOG_LAST_FATAL();
    }
    ...
    worker.start();
    worker.submit([=] {
        *diagnostic = "first command diagnostic";
        return result;
    });
    worker.submit([=] {
        return result;
    ...
    CompositorWorkerDouble::CompletedCommand command;
    REQUIRE(worker.take(command));
    REQUIRE(command.result == result);
    REQUIRE(command.message == "first command diagnostic");

    REQUIRE(worker.take(command));
    REQUIRE(command.result == result);
    // Current defect: the process-global warning/fatal diagnostic leaks into
    // a later command that did not report one.
    REQUIRE(command.message.empty());
    REQUIRE_FALSE(worker.take(command));
}
