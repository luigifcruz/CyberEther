...
void RequireConnection(const Flowgraph::View::BlockData& block,
                       const std::string& input,
                       const std::string& source,
                       const std::string& output) {
    REQUIRE(block.inputs.contains(input));
    REQUIRE(block.inputs.at(input).external.has_value());
    REQUIRE(block.inputs.at(input).external->block == source);
    REQUIRE(block.inputs.at(input).external->port == output);
}

void RequireNoConnection(const Flowgraph::View::BlockData& block,
                         const std::string& input) {
    REQUIRE_FALSE(block.inputs.contains(input));
}
...
TEST_CASE_METHOD(FlowgraphFixture, "Block creation and destruction", "[core][flowgraph]") {
    SECTION("create single block") {
        auto result = flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {});
    ...
    SECTION("create multiple blocks") {
        REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
        REQUIRE(flowgraph->blockCreate("gen2", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    ...
    SECTION("create duplicate block fails") {
        REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
        REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::ERROR);
    }
...
TEST_CASE_METHOD(FlowgraphFixture,
                 "Known faulting block types remain inspectable as errored",
    ...
    SECTION("block define failure") {
        ...
        REQUIRE(faults.blockDefineCalls == 1);
    }

    SECTION("block create failure") {
        faults.failNext(SyntheticFaultPoint::BlockCreate);
        REQUIRE(flowgraph->blockCreate("fault", kSyntheticFaultType, {}, {}) == Result::SUCCESS);

        const auto block = viewBlock("fault");
        REQUIRE(block.type == kSyntheticFaultType);
        REQUIRE(block.state == Block::State::Errored);
        REQUIRE(block.diagnostic.find("Forced block create failure") != std::string::npos);
        REQUIRE(block.outputs.empty());
        REQUIRE(faults.blockCreateCalls == 1);
    }

    SECTION("module define failure") {
        faults.failNext(SyntheticFaultPoint::ModuleDefine);
        REQUIRE(flowgraph->blockCreate("fault", kSyntheticFaultType, {}, {}) == Result::SUCCESS);

        const auto block = viewBlock("fault");
        REQUIRE(block.type == kSyntheticFaultType);
        REQUIRE(block.state == Block::State::Errored);
        REQUIRE(block.diagnostic.find("Forced module define failure") != std::string::npos);
        REQUIRE(block.outputs.empty());
    ...
    SECTION("module create failure") {
        ...
        REQUIRE(block.type == kSyntheticFaultType);
        REQUIRE(block.state == Block::State::Errored);
        REQUIRE(block.diagnostic.find("Forced module create failure") != std::string::npos);
        REQUIRE(block.outputs.empty());
...
TEST_CASE_METHOD(FlowgraphFixture,
                 "Present failure marks the owning block errored",
                 "[core][flowgraph][present][fault]") {
    auto& faults = syntheticFaultState();

    REQUIRE(flowgraph->blockCreate("fault", kSyntheticFaultType, {}, {}) == Result::SUCCESS);
    TensorMap sinkInputs;
    sinkInputs["buffer"].requested("fault", "out");
    REQUIRE(flowgraph->blockCreate("sink", kSyntheticPassType, {}, sinkInputs) == Result::SUCCESS);

    faults.failNext(SyntheticFaultPoint::ModulePresent);
    REQUIRE(flowgraph->present() == Result::SUCCESS);

    const auto errored = viewBlock("fault");
    REQUIRE(errored.state == Block::State::Errored);
    REQUIRE(errored.diagnostic.find("Forced module present failure") != std::string::npos);
    REQUIRE(errored.outputs.empty());
    REQUIRE(viewBlock("sink").state == Block::State::Incomplete);
    REQUIRE(faults.modulePresentCalls == 1);

    REQUIRE(flowgraph->present() == Result::SUCCESS);
    REQUIRE(faults.modulePresentCalls == 1);
}

TEST_CASE_METHOD(FlowgraphFixture, "Environment change retries incomplete blocks", "[core][flowgraph][environment]") {
...
TEST_CASE_METHOD(FlowgraphFixture, "Environment change retries incomplete downstream chain", "[core][flowgraph][environment]") {
    auto& state = environmentGateTestState();
    state.reset();
...
TEST_CASE_METHOD(FlowgraphFixture, "Environment change skips unconnected incomplete blocks", "[core][flowgraph][environment]") {
    auto& state = environmentGateTestState();
    state.reset();
...
TEST_CASE_METHOD(FlowgraphFixture, "Environment value update does not retry incomplete blocks", "[core][flowgraph][environment]") {
    auto& state = environmentGateTestState();
    state.reset();
...
TEST_CASE_METHOD(FlowgraphFixture,
                 "Environment key removal retries incomplete blocks once",
                 "[core][flowgraph][environment][clear]") {
    auto& state = environmentGateTestState();
    state.reset();
    ...
    SimpleMetaFixture trigger;
    trigger.label = "trigger";
    REQUIRE(flowgraph->environment().set("trigger", trigger) == Result::SUCCESS);
    REQUIRE(flowgraph->compute() == Result::SUCCESS);
    REQUIRE(state.createCalls == 2);

    REQUIRE(flowgraph->environment().clear("trigger") == Result::SUCCESS);
    REQUIRE(flowgraph->compute() == Result::SUCCESS);
    REQUIRE(viewBlock("gated").state == Block::State::Incomplete);
    REQUIRE(state.createCalls == 3);
    ...
    REQUIRE(flowgraph->environment().set("trigger", trigger) == Result::SUCCESS);
    REQUIRE(flowgraph->compute() == Result::SUCCESS);
    REQUIRE(state.createCalls == 4);
    REQUIRE(flowgraph->environment().clearAll() == Result::SUCCESS);
    REQUIRE(flowgraph->compute() == Result::SUCCESS);
    REQUIRE(state.createCalls == 5);
    ...
    REQUIRE(flowgraph->compute() == Result::SUCCESS);
    REQUIRE(viewBlock("gated").state == Block::State::Created);
    REQUIRE(state.createCalls == 6);
}

TEST_CASE_METHOD(FlowgraphFixture, "Block connection", "[core][flowgraph]") {
    REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    REQUIRE(flowgraph->blockCreate("gen2", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    REQUIRE(flowgraph->blockCreate("gen3", kSyntheticSourceType, {}, {}) == Result::SUCCESS);

    TensorMap addInputs;
    addInputs["a"].requested("gen1", "signal");
    addInputs["b"].requested("gen2", "signal");
    REQUIRE(flowgraph->blockCreate("add1", kSyntheticMergeType, {}, addInputs) == Result::SUCCESS);
...
TEST_CASE_METHOD(FlowgraphFixture, "Downstream propagation on connect", "[core][flowgraph][propagation]") {
    ...
    REQUIRE(flowgraph->blockCreate("add1", kSyntheticMergeType, {}, add1Inputs) == Result::SUCCESS);

    // Connect add1 -> add2.a
    TensorMap add2Inputs;
    add2Inputs["a"].requested("add1", "sum");
    add2Inputs["b"].requested("gen2", "signal");
    REQUIRE(flowgraph->blockCreate("add2", kSyntheticMergeType, {}, add2Inputs) == Result::SUCCESS);
...
TEST_CASE_METHOD(FlowgraphFixture, "Downstream propagation on destroy", "[core][flowgraph][propagation]") {
    // Create chain: gen1 -> add1 -> add2
    REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    REQUIRE(flowgraph->blockCreate("gen2", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    ...
    SECTION("destroying upstream marks downstream incomplete") {
        ...
        const auto add1 = viewBlock("add1");
        REQUIRE(add1.state == Block::State::Incomplete);
        RequireNoConnection(add1, "a");
        RequireConnection(add1, "b", "gen2", "signal");

        const auto add2 = viewBlock("add2");
        REQUIRE(add2.state == Block::State::Incomplete);
        RequireConnection(add2, "a", "add1", "sum");
        RequireConnection(add2, "b", "gen2", "signal");
    }
    ...
    SECTION("destroying all upstream sources marks downstream incomplete") {
        ...
        REQUIRE(add1.state == Block::State::Incomplete);
        RequireNoConnection(add1, "a");
        RequireNoConnection(add1, "b");

        const auto add2 = viewBlock("add2");
        REQUIRE(add2.state == Block::State::Incomplete);
        RequireConnection(add2, "a", "add1", "sum");
        RequireNoConnection(add2, "b");
    }
}

TEST_CASE_METHOD(FlowgraphFixture, "Destroy block with multi-input downstream", "[core][flowgraph][propagation]") {
    // Topology:
    //   gen1 ──► add0 ──┬──► add1 ──► add2
    //   gen2 ───────────┘
    //
    ...
    REQUIRE(flowgraph->blockCreate("add0", kSyntheticMergeType, {}, add0Inputs) == Result::SUCCESS);
    ...
    SECTION("reconnecting after middle block destruction restores chain") {
        ...
        const auto add1 = viewBlock("add1");
        REQUIRE(add1.state == Block::State::Created);
        RequireConnection(add1, "a", "gen1", "signal");
        RequireConnection(add1, "b", "gen2", "signal");

        const auto add2 = viewBlock("add2");
        REQUIRE(add2.state == Block::State::Created);
        RequireConnection(add2, "a", "add1", "sum");
        RequireConnection(add2, "b", "gen2", "signal");
    }
}

TEST_CASE_METHOD(FlowgraphFixture, "Destroy block with deep downstream chain", "[core][flowgraph][propagation]") {
    ...
    REQUIRE(flowgraph->blockCreate("add3", kSyntheticMergeType, {}, add3Inputs) == Result::SUCCESS);

    TensorMap add4Inputs;
    add4Inputs["a"].requested("add3", "sum");
    add4Inputs["b"].requested("gen2", "signal");
    REQUIRE(flowgraph->blockCreate("add4", kSyntheticMergeType, {}, add4Inputs) == Result::SUCCESS);
...
TEST_CASE_METHOD(FlowgraphFixture, "Destroy block with diamond dependency", "[core][flowgraph][propagation]") {
    // Topology (diamond):
    //        ┌──► add1 ──┐
    // gen1 ──┤           ├──► add3
    //        └──► add2 ──┘
...
TEST_CASE_METHOD(FlowgraphFixture, "Destroy block with multiple dependents at same level", "[core][flowgraph][propagation]") {
    // Topology:
    //        ┌──► add1
    // add0 ──┼──► add2
    //        └──► add3
...
TEST_CASE_METHOD(FlowgraphFixture, "Destroy with no propagation flag", "[core][flowgraph][propagation]") {
...
TEST_CASE_METHOD(FlowgraphFixture, "Destroy block then recreate with same name", "[core][flowgraph][propagation]") {
    REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    REQUIRE(flowgraph->blockCreate("gen2", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
...
TEST_CASE_METHOD(FlowgraphFixture, "Destroy source block used by multiple inputs of same block", "[core][flowgraph][propagation]") {
...
TEST_CASE_METHOD(FlowgraphFixture, "Fan-out propagation", "[core][flowgraph][propagation]") {
    // gen1 -> add1
    // gen1 -> add2
    REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    REQUIRE(flowgraph->blockCreate("gen2", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
...
TEST_CASE_METHOD(FlowgraphFixture, "Block reconfiguration", "[core][flowgraph]") {
    REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    REQUIRE(viewBlock("gen1").state == Block::State::Created);
...
TEST_CASE_METHOD(FlowgraphFixture, "Block recreation", "[core][flowgraph][recreation]") {
    REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    REQUIRE(viewBlock("gen1").state == Block::State::Created);
...
TEST_CASE_METHOD(FlowgraphFixture, "Block recreation can change implementation", "[core][flowgraph][recreation]") {
    REQUIRE(RegisterSyntheticGraph() == Result::SUCCESS);
    REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    ...
    const auto implementations = Registry::ListAvailableModules(kSyntheticSourceType,
                                                                 block.device,
                                                                 block.runtime,
...
TEST_CASE_METHOD(FlowgraphFixture, "Block recreation with downstream chain", "[core][flowgraph][recreation]") {
...
TEST_CASE_METHOD(FlowgraphFixture, "Block recreation with diamond dependency", "[core][flowgraph][recreation]") {
    // Topology (diamond):
    //        ┌──► add1 ──┐
    // gen1 ──┤           ├──► add3
    //        └──► add2 ──┘
...
TEST_CASE_METHOD(FlowgraphFixture,
    ...
    REQUIRE(flowgraph->blockCreate("short", kSyntheticPassType, {}, shortInputs) == Result::SUCCESS);

    TensorMap long1Inputs;
    long1Inputs["buffer"].requested("gen1", "signal");
    REQUIRE(flowgraph->blockCreate("long1", kSyntheticPassType, {}, long1Inputs) == Result::SUCCESS);

    TensorMap long2Inputs;
    long2Inputs["buffer"].requested("long1", "buffer");
    REQUIRE(flowgraph->blockCreate("long2", kSyntheticPassType, {}, long2Inputs) == Result::SUCCESS);

    TensorMap long3Inputs;
    long3Inputs["buffer"].requested("long2", "buffer");
    REQUIRE(flowgraph->blockCreate("long3", kSyntheticPassType, {}, long3Inputs) == Result::SUCCESS);
...
TEST_CASE_METHOD(FlowgraphFixture, "Block recreation with fan-out", "[core][flowgraph][recreation]") {
    // Topology:
    //        ┌──► add1
    // add0 ──┼──► add2
    //        └──► add3
...
TEST_CASE_METHOD(FlowgraphFixture, "Block recreation preserves connections", "[core][flowgraph][recreation]") {
...
TEST_CASE_METHOD(FlowgraphFixture, "Flowgraph serialization", "[core][flowgraph][serialization]") {
    REQUIRE(flowgraph->blockCreate("gen1", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
    REQUIRE(flowgraph->blockCreate("gen2", kSyntheticSourceType, {}, {}) == Result::SUCCESS);
