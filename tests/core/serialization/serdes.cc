...
TEST_CASE("JST_SERDES serializes nested structs as maps", "[core][serialization][serdes]") {
    OuterConfig source;
...
TEST_CASE("JST_SERDES cascades string-backed nested maps", "[core][serialization][serdes]") {
    Parser::Map data;
...
TEST_CASE("JST_SERDES round-trips complex fields", "[core][serialization][serdes]") {
...
TEST_CASE("JST_SERDES decodes string-backed complex fields", "[core][serialization][serdes]") {
    Parser::Map data;
...
TEST_CASE("JST_SERDES serializes vectors of nested structs as sequences", "[core][serialization][serdes]") {
...
TEST_CASE("JST_SERDES deserializes string-backed sequences of nested structs", "[core][serialization][serdes]") {
...
TEST_CASE("JST_SERDES omits null optional fields during serialization", "[core][serialization][serdes]") {
...
TEST_CASE("JST_SERDES resets missing optional fields during deserialization", "[core][serialization][serdes]") {
...
TEST_CASE("JST_SERDES round-trips present optional fields", "[core][serialization][serdes]") {
    OptionalConfig source;
...
TEST_CASE("Parser stores primitive vectors directly and nested vectors as sequences", "[core][serialization][serdes]") {
...
TEST_CASE("ParserMap preserves insertion order and repairs indices after erase", "[core][serialization][serdes][map]") {
    Parser::Map data;
    data.reserve(4);
    data["alpha"] = U64{1};
    data["beta"] = U64{2};
    ...
    data["alpha"] = U64{9};
    REQUIRE(data.size() == 3);
    REQUIRE(std::any_cast<U64>(data.at("alpha")) == 9);

    data.erase("beta");
    REQUIRE(data.size() == 2);
    REQUIRE(!data.contains("beta"));
    REQUIRE(std::any_cast<U64>(data.at("gamma")) == 3);
    ...
    REQUIRE(keys == std::vector<std::string>({"alpha", "gamma", "delta"}));

    data.erase("missing");
    REQUIRE(data.size() == 3);
    REQUIRE_THROWS_AS(data.at("missing"), std::out_of_range);
...
TEST_CASE("Parser::Serialize overwrites and erases existing entries", "[core][serialization][serdes]") {
...
TEST_CASE("Parser::Deserialize uses exact-type fast paths", "[core][serialization][serdes]") {
    Parser::Map data;
    data["gain"] = U64{17};
...
TEST_CASE("Parser::Deserialize handles missing and uninitialized values", "[core][serialization][serdes]") {
...
TEST_CASE("Parser::Deserialize reports incompatible types", "[core][serialization][serdes]") {
    Parser::Map data;
...
TEST_CASE("Parser sequence deserialization is atomic on nested errors", "[core][serialization][serdes][errors]") {
    Parser::Sequence encoded;
    encoded.push_back(MakeInnerMap(1, true));
    encoded.push_back(U64{2});

    Parser::Map data;
    data["steps"] = encoded;

    SequenceConfig restored;
    restored.steps.push_back({.gain = 99, .enabled = false});
    REQUIRE(restored.deserialize(data) == Result::ERROR);
    REQUIRE(restored.steps.size() == 1);
    REQUIRE(restored.steps.at(0).gain == 99);
    REQUIRE(!restored.steps.at(0).enabled);
}

TEST_CASE("Parser optional deserialization preserves values on type errors", "[core][serialization][serdes][errors]") {
    Parser::Map data;
    data["label"] = U64{7};

    OptionalConfig restored;
    restored.label = "unchanged";
    REQUIRE(restored.deserialize(data) == Result::ERROR);
    REQUIRE(restored.label == std::optional<std::string>{"unchanged"});
}

TEST_CASE("Parser::Serialize propagates custom serializer failures", "[core][serialization][serdes][errors]") {
    Parser::Map data;
    data["value"] = std::string("unchanged");

    REQUIRE(Parser::Serialize(data, "value", FailingSerializeConfig{}) == Result::ERROR);
    REQUIRE(std::any_cast<std::string>(data.at("value")) == "unchanged");
    REQUIRE(Parser::Serialize(data, "throwing", ThrowingConfig{}) == Result::ERROR);
    REQUIRE(!data.contains("throwing"));
}
