...
std::vector<const char*> WebGPU::getRequiredInstanceExtensions() {
...
std::vector<const char*> WebGPU::getRequiredValidationLayers() {
...
std::vector<const char*> WebGPU::getRequiredDeviceExtensions() {
    std::vector<const char*> extensions;
...
bool WebGPU::checkValidationLayerSupport() {
    // uint32_t layerCount;
...
// bool WebGPU::checkDeviceExtensionSupport(const VkPhysicalDevice& device) {
...
EM_JS(WGPUDevice, wgpuAdapterRequestDeviceSync, (WGPUAdapter adapterId), {
    return Module['preinitializedWebGPUDevice'];
});

WebGPU::WebGPU(const Config& _config) : config(_config) {
    // Create application.


    JST_INFO("Loading");
    WGPUAdapter ad = wgpuInstanceRequestAdapterSync();
    adapter = wgpu::Adapter::Acquire(ad);
    device = wgpu::Device::Acquire(wgpuAdapterRequestDeviceSync(ad));
    JST_INFO("Loaded");
    ...
    surface = instance.CreateSurface(&surface_desc).Release();
