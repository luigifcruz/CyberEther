...
struct Decimator : public Block::Config {
    ...
    JST_BLOCK_PARAMS(ratio);
    JST_BLOCK_DESCRIPTION(
        "Decimator",
        "Decimates a signal along its sample axis.",
        "# Decimator\n"
        "The Decimator block reduces the sample count "
        ...
        "preserving accumulated energy. The input must identify its sample "
        "dimension with `sampleAxis`. Optional `batchAxis` and `channelAxis` roles "
        ...
        "1. Reshape module separates the sample axis into ratio chunks.\n"
