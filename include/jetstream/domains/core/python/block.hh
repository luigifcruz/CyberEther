...
struct Python : public Block::Config {
    ...
    JST_BLOCK_DESCRIPTION(
        ...
        "downstream blocks stay informed. Signal axes declared in the output "
        "tensor spec (as `[B, C, S]`, next to the shape) are published when "
        "the block is created, before downstream blocks validate. Each role "
        "sits at the position of its axis (`B` batch, `C` channel, `S` "
        "sample). Use `_` for an axis with no role and omit trailing axes "
        "entirely. Declared roles are immutable: compute() writes to "
        "`sampleAxis`, `batchAxis`, or `channelAxis` are ignored.\n\n"
