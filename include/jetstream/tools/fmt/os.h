...
JST_FMT_API std::system_error vwindows_error(int error_code, string_view fmt,
                                         format_args args);
...
struct buffer_size {
  ...
  size_t value = 0;
  JST_FMT_CONSTEXPR auto operator=(size_t val) const -> buffer_size {
    auto bs = buffer_size();
    bs.value = val;
    return bs;
  }
...
JST_FMT_INLINE_VARIABLE constexpr auto buffer_size = detail::buffer_size();

/// A fast buffered output stream for writing from a single thread. Writing from
/// multiple threads without external synchronization may result in a data race.
class JST_FMT_API ostream : private detail::buffer<char> {
 private:
  file file_;
