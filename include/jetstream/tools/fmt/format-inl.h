...
JST_FMT_FUNC void do_report_error(format_func func, int error_code,
...
JST_FMT_FUNC void report_error(const char* message) {
  ...
  // Use JST_FMT_THROW instead of throw to avoid bogus unreachable code warnings
  // from MSVC.
...
JST_FMT_INLINE_VARIABLE constexpr struct div_small_pow10_infos_struct {
  uint32_t divisor;
  int shift_amount;
...
JST_FMT_FUNC void vformat_to(buffer<char>& buf, string_view fmt, format_args args,
                         locale_ref loc) {
...
#  define JST_FMT_USE_FALLBACK_FILE 0
#endif
