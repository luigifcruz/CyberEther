...
void NodeRuntimeOverlay::render(const Context& ctx) const {
    ...
    const ImU32 primaryColor = ImGui::ColorConvertFloat4ToU32(Private::ImColor(ctx, config.primaryColorKey));
    const ImU32 secondaryColor = ImGui::ColorConvertFloat4ToU32(Private::ImColor(ctx, config.secondaryColorKey));
